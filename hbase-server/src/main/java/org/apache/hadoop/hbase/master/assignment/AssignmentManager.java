...
public class AssignmentManager {
    ...
    void persistToMeta(RegionStateNode regionNode) throws IOException {
        regionStateStore.updateRegionLocation(regionNode);
        RegionInfo regionInfo = regionNode.getRegionInfo();
        System.out.println("Persisting to meta: " + regionInfo +" isDryRun is " + PilotUtil.isDryRun());
    ...
    public LockWrapper<AssignmentManager> assignQueueLock$dryrun = new LockWrapper<>(assignQueueLock,this);
    private final Condition assignQueueFullCond = assignQueueLock.newCondition();

    public final ConditionVariableWrapper<AssignmentManager> assignQueueFullCond$dryrun = new ConditionVariableWrapper<AssignmentManager>(assignQueueFullCond, assignQueueLock, this);
    ...
    protected void queueAssign$instrumentation(final RegionStateNode regionNode) {
        ...
        try {
            ...
            LOG.info("Region added to pendingAssignQueue$dryrun, size: " + pendingAssignQueue$dryrun.size() + " isDryRun is" + PilotUtil.isDryRun()+"hashcode is"+pendingAssignQueue$dryrun.hashCode());
            if (
...
//  }
    ...
    private HashMap<RegionInfo, RegionStateNode> waitOnAssignQueue() {
        ...
        //record state
        assignQueueLock$dryrun.lock();
        LOG.info("acquired lock isDryRun is" + PilotUtil.isDryRun() + "isShadow is" + PilotUtil.isShadow() + "size is" + pendingAssignQueue$dryrun.size());
        try {
            if (pendingAssignQueue.isEmpty() && isRunning()) {
                //record state
                LOG.info("wait on cv" + PilotUtil.isDryRun() + "isShadow is" + PilotUtil.isShadow() + "size is" + pendingAssignQueue$dryrun.size());
                assignQueueFullCond$dryrun.await();
            }
            LOG.info("release on cv" + PilotUtil.isDryRun() + "isShadow is" + PilotUtil.isShadow() + "size is" + pendingAssignQueue$dryrun.size());

            LOG.info(" thread woke up isDryRun is" + PilotUtil.isDryRun());
            ...
            LOG.info("release on cv await time" + PilotUtil.isDryRun() + "isShadow is" + PilotUtil.isShadow() + "size is" + pendingAssignQueue$dryrun.size());
            ...
            LOG.info("Shadow thread processing regions, size: " + regions.size() +" isDryRun is" + PilotUtil.isDryRun());
            pendingAssignQueue.clear();
        } catch (InterruptedException e) {
...
//  }
    ...
    private void processAssignmentPlans(final HashMap<RegionInfo, RegionStateNode> regions,
        ...
        if (retainMap != null && !retainMap.isEmpty()) {
            ...
            try {
                LOG.info("Retain assignment for regions, size: " + retainMap.size() + " isDryRun is" + PilotUtil.isDryRun());
        ...
        if (!hris.isEmpty()) {
            ...
            try {
                LOG.info("Round-robin assignment for regions, size: " + hris.size() + " isDryRun is" + PilotUtil.isDryRun());
    ...
    private void acceptPlan(final HashMap<RegionInfo, RegionStateNode> regions,
        ...
        for (Map.Entry<ServerName, List<RegionInfo>> entry : plan.entrySet()) {
            ...
            for (RegionInfo hri : entry.getValue()) {
                ...
                LOG.info("Assigning region " + hri.getRegionNameAsString() + " to " + server + "isDryRun is" + PilotUtil.isDryRun());
    ...
    private void addToPendingAssignment(final HashMap<RegionInfo, RegionStateNode> regions,
        ...
        LOG.info("Adding regions to pending assignment, size: " + pendingRegions.size() + " isDryRun is" + PilotUtil.isDryRun());
        assignQueueLock.lock();
        try {
